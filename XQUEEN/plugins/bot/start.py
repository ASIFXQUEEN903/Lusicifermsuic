...
from XQUEEN.misc import _boot_  
from XQUEEN.plugins.sudo.sudoers import sudoers_list  
from XQUEEN.utils.database import (  
    add_served_chat,  
    add_served_user,  
    blacklisted_chats,  
    get_lang,  
...
from XQUEEN.utils.decorators.language import LanguageStart  
from XQUEEN.utils.formatters import get_readable_time  
from XQUEEN.utils.inline import help_pannel, private_panel, start_panel  
    ...
    if len(message.text.split()) > 1:  
        ...
        if name[:8] == "connect_":  
            ...
            if message.from_user.id not in admin_ids:  
                return await message.reply_text(f"sᴏʀʀʏ sɪʀ ʙᴜᴛ ɪ ᴛʜᴀᴛ ʏᴏᴜ ɴᴏᴛ ᴀɴ ᴀᴅᴍɪɴ ᴏғ {title}")  
        ...
        if name[0:3] == "sud":  
            ...
            if await is_on_off(2):  
                return await app.send_message(  
                    ...
                    text=f"{message.from_user.mention} ᴊᴜsᴛ sᴛᴀʀᴛᴇᴅ ᴛʜᴇ ʙᴏᴛ ᴛᴏ ᴄᴇᴄᴋ <b>sᴜᴅᴏʟɪsᴛ</b>.\n\n<b>ᴜsᴇʀ ɪᴅ :</b> <code>{message.from_user.id}</code>\n<b>ᴜsᴇʀɴᴀᴍᴇ :</b> @{message.from_user.username}",  
                )  
            return  
        if name[0:3] == "inf":  
