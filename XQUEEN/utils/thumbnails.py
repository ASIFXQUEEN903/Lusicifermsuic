...
async def get_thumb(videoid):
    ...
    try:
        ...
        # Load assets
        template = Image.open("XQUEEN/assets/thum.png").convert("RGBA")  # size 1280x720
        ...
        # Circular crop
        crop_size = 390  # fit perfectly inside the white ring
        ...
        thumb.putalpha(mask)
        final.paste(thumb, (105, 165), mask=thumb)  # align circle manually

        # Paste overlay template
        ...
        draw.text((530, 20), clear(title), fill="white", font=font_title)                     # title above line
        draw.text((530, 350), f"00:00 / {duration}", fill="white", font=font_small)           # duration
        draw.text((1200, 690), "XQUEEN SERVER", fill="white", font=font_small)                # tag
